...
const StepCard: React.FC<StepCardProps> = ({ step, index, isLast }) => {
  return (
    <div className="relative">
          ...
          <div className="flex-shrink-0">
            <div className="relative">
              ...
              <div className="absolute -bottom-1.5 -right-1.5 md:-bottom-2 md:-right-2 w-7 h-7 md:w-8 md:h-8 bg-secondary rounded-full flex items-center justify-center text-white text-xs md:text-sm">
                <i 
                  className={`${
                    step.icon === 'user-plus' ? 'fas fa-user-plus' :
                    step.icon === 'camera' ? 'fas fa-camera' :
                    step.icon === 'handshake' ? 'fas fa-handshake' : ''
                  }`}
                  aria-hidden="true"
                />
              </div>
      ...
      {!isLast && (
        <div className="hidden md:block absolute top-7 md:top-8 left-7 md:left-8 w-0.5 h-20 md:h-24 bg-gradient-to-b from-primary to-primary/30 transform translate-y-8" />
      )}
    </div>
  );
};
      ...
      <Container size="lg">
        <AnimatedSection animation="fade">
          <div className="text-center mb-10 md:mb-16 px-4">
            ...
            <p className="text-base sm:text-lg md:text-xl text-textSecondary max-w-3xl mx-auto">
              {subtitle}
            </p>
          </div>
        </AnimatedSection>
